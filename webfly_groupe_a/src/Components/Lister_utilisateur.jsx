...
function Lister_utilisateur() {
    ...
    const handleDeleteUser = async (userId) => {
        try {
            await axios.delete(`http://localhost:3001/sup_utilisateur/${userId}`);

            const response = await fetch("http://localhost:3001/lister_utilisateur");
            if (response.ok) {
                const data = await response.json();
                setUserData(data);
            } else {
                console.error("Erreur lors de la récupération des données après la suppression");
            }
        } catch (error) {
        ...
        try {
            ...
            if (success) {
                ...
                const updatedUserData = userData.map(user => {
                    if (user.id === userId) {
                        return { ...user, nom: editedValues.nom, email: editedValues.email };
                    }
                    return user;
                });
                ...
                <div className='user' key={user.id}>
                    {editingUserId === user.id ? (
                        <>
                            ...
                            <td>
                                <button onClick={() => handleUpdateUser(user.id)}>Envoyer la modification</button>
                            </td>
                            <td>
                        ...
                        <>
                            <td>{user.nom}</td>
                            <td>{user.email}</td>
                            <td><button onClick={() => handleEditUser(user.id)}>Modifier</button></td>
                            <td><button onClick={() => handleDeleteUser(user.id)}>Supprimer</button></td>
                        </>
                    )}
                </div>
            ))}
...
export default Lister_utilisateur;