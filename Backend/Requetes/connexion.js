    ...
    connection.query(sql, [mail], (error, rows, fields) => {
      if (error) {
        ...
        if (rows.length === 0) {
          ...
          connection.query(
            "SELECT ID_UTILISATEUR, NOM, PRENOM, MDP, ENTREPRISE, ROLE_UTILISATEUR FROM utilisateur WHERE ID_UTILISATEUR = ?",
...
const etatEnLigne = (app, connection) => {
  app.put("/connexion/:id", (req, res) => {
    ...
    console.log(idUser);
    connection.query(
      "UPDATE utilisateur SET ETAT = ?  WHERE ID_UTILISATEUR = ?",
      ["En Ligne", idUser],
      function (err, result) {
        if (err) {
